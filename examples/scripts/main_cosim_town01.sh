...
# CARLA host: Docker 内から Windows CARLA には localhost が通らないので
# Docker Desktop の host.docker.internal(DNS 名、再起動耐性あり)を既定とする。
# 環境依存で解決しない場合は CARLA_HOST 環境変数で上書き。
# ([docs/decisions/cosim-architecture.md](../../../docs/decisions/cosim-architecture.md) 参照)
...
if [ ! -f /app/examples/maps/town01/Town01.net.xml ]; then
    ...
    echo "  Generate it via netconvert from Town01.xodr (see docs/setup/phase1.md)"
    exit 1
fi
echo "  Map files OK."
