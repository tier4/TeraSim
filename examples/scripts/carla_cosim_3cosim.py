...
Difference from carla_cosim_main.py:
  ...
  - map_name empty : do NOT reload the world (psim already loaded odaibatest5)
...
Usage:
  python carla_cosim_3cosim.py \
      --terasim_config examples/scenarios/cosim_odaiba_ll2.yaml \
      --carla_port 2013
"""
import argparse
...
def main():
    ...
    p.add_argument('--carla_timeout', default=600.0, type=float,
                   help='CARLA client timeout; large because odaibatest5 to_opendrive is ~76MB')
    ...
    p.add_argument('--terasim_config', required=True,
                   help='TeraSim scenario yaml (e.g. examples/scenarios/cosim_odaiba_ll2.yaml)')
